        ...
        <ComponentRow className="row-item" {...rowopts}>
          {formFields.map((field, i) => {
            ...
            switch (field.formItem.type) {
              ...
              case 'month':
              case 'time':
                FieldComp = require(`./components/date`).default(formProps);
                break;
              case 'input':
              case 'hidden':
              case 'textarea':
                FieldComp = require(`./components/input`).default(formProps);
                break;
              default:
                FieldComp = require(`./components/${type.toLowerCase()}`).default(
                  formProps
                );
                break;
            }
            ...
            switch (field.formItem.type) {
              ...
              case 'upload':
                const ph =
                  field.formItem.type === 'autoComplete' ? '请输入' : '请选择';
                const selectProps = {
                  form: form,
                  dict: field.dict,
                  allowClear: true,
